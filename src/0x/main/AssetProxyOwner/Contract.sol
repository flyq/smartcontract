...
contract MultiSigWalletWithTimeLock is MultiSigWallet {
    ...
    modifier pastTimeLock(uint256 transactionId) {
        require(
            block.timestamp >=
                confirmationTimes[transactionId] + secondsTimeLocked,
...
contract AssetProxyOwner is MultiSigWalletWithTimeLock {
    ...
    bytes4 internal constant REMOVE_AUTHORIZED_ADDRESS_AT_INDEX_SELECTOR =
        bytes4(keccak256("removeAuthorizedAddressAtIndex(address,uint256)"));
        ...
        MultiSigWalletWithTimeLock(_owners, _required, _secondsTimeLocked)
    {
        for (uint256 i = 0; i < _assetProxyContracts.length; i++) {
            ...
            require(assetProxy != address(0), "INVALID_ASSET_PROXY");
